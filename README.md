...
timer); both add `python -m src.weather_divergence` (forecast vs the
temperature the band prices imply, station by station). Server bootstrap: `deploy/engines_setup.sh` then `deploy/deploy.sh`
